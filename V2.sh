...
v2ray_hello(){
	echo ""
	echo -e "${Info} ${Blue} 你正在执行 V2RAY 基于 NGINX 的 VMESS+WS+TLS+Website(Use Host) 一键安装脚本 ${Font}"
	echo ""
}
...
is_root(){
	if [ `id -u` == 0 ]
		then echo -e "${OK} ${Blue} 当前用户是root用户，开始安装流程 ${Font} "
		sleep 3
	else
...
check_system(){
	...
	if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]];then
		echo -e "${OK} ${Blue} 当前系统为 Centos ${VERSION_ID} ${VERSION} ${Font} "
		INS="yum"
		echo -e "${OK} ${Blue} SElinux 设置中，请耐心等待，不要进行其他操作${Font} "
		setsebool -P httpd_can_network_connect 1 >/dev/null 2>&1
		echo -e "${OK} ${Blue} SElinux 设置完成 ${Font} "
		## 添加 Nginx apt源
...
enabled=1
EOF
		echo -e "${OK} ${Blue} Nginx 源 安装完成 ${Font}"
	elif [[ "${ID}" == "debian" && ${VERSION_ID} -ge 8 ]];then
		echo -e "${OK} ${Blue} 当前系统为 Debian ${VERSION_ID} ${VERSION} ${Font} "
		INS="apt"
		## 添加 Nginx apt源
		...
		echo -e "${OK} ${Blue} 当前系统为 Ubuntu ${VERSION_ID} ${VERSION_CODENAME} ${Font} "
		INS="apt"
		## 添加 Nginx apt源
		if [ ! -f nginx_signing.key ];then
...
#用户设定 域名 端口 alterID
port_alterid_set(){
	echo -e "${Info} ${Blue} 【配置 1/3 】请输入你的域名信息(如:www.bing.com)，请确保域名A记录已正确解析至服务器IP ${Font}"
	stty erase '^H' && read -p "请输入：" domain
	echo -e "${Info} ${Blue} 【配置 2/3 】请输入连接端口（默认:443 无特殊需求请直接按回车键） ${Font}"
	stty erase '^H' && read -p "请输入：" port
	[[ -z ${port} ]] && port="443"
	echo -e "${Info} ${Blue} 【配置 3/3 】请输入alterID（默认:64 无特殊需求请直接按回车键） ${Font}"
	stty erase '^H' && read -p "请输入：" alterID
	[[ -z ${alterID} ]] && alterID="64"
	echo -e "----------------------------------------------------------"
	echo -e "${Info} ${Blue} 你输入的配置信息为 域名：${domain} 端口：${port} alterID：${alterID} ${Font}"
	echo -e "----------------------------------------------------------"
}

#强制清除可能残余的http服务 v2ray服务 关闭防火墙 更新源
apache_uninstall(){
	echo -e "${Info} ${Blue} 正在强制清理可能残余的http服务 ${Font}"
	if [[ "${ID}" == "centos" ]];then
	...
	echo -e "${Info} ${Blue} 正在更新源 请稍后 …… ${Font}"
...
domain_check(){
	...
	echo -e "${OK} ${Blue} 正在获取 公网ip 信息，请耐心等待 ${Font}"
	local_ip=`curl -4 ip.sb`
	echo -e "${OK} ${Blue} 域名dns解析IP：${domain_ip} ${Font}"
	echo -e "${OK} ${Blue} 本机IP: ${local_ip} ${Font}"
	sleep 2
	if [[ $(echo ${local_ip}|tr '.' '+'|bc) -eq $(echo ${domain_ip}|tr '.' '+'|bc) ]];then
		echo -e "${OK} ${Blue} 域名dns解析IP  与 本机IP 匹配 域名解析正确 ${Font}"
		sleep 2
	else
...
port_exist_check(){
	if [[ 0 -eq `lsof -i:"$1" | wc -l` ]];then
		...
		echo -e "${Error} ${Red} 检测到 $1 端口被占用，以下为 $1 端口占用信息 ${Font}"
		lsof -i:"$1"
		echo -e "${OK} ${Blue} 5s 后将尝试自动 kill 占用进程 ${Font}"
		sleep 5
		lsof -i:"$1" | awk '{print $2}'| grep -v "PID" | xargs kill -9
		echo -e "${OK} ${Blue} kill 完成 ${Font}"
		sleep 1
	fi
}
...
time_modify(){
	...
	echo -e "${Info} ${Blue} 正在进行时间同步 ${Font}"
	ntpdate time.nist.gov

	if [[ $? -eq 0 ]];then 
		echo -e "${OK} ${Blue} 时间同步成功 ${Font}"
		echo -e "${OK} ${Blue} 当前系统时间 `date -R`（时区时间换算后误差应为三分钟以内）${Font}"
		sleep 1
	else
		echo -e "${Error} ${Red} 时间同步失败，请检查ntpdate服务是否正常工作 ${Font}"
	fi 
}
...
v2ray_install(){
	...
	if [[ -f go.sh ]];then
		...
		echo -e "${Error} ${Red} V2ray 安装文件下载失败，请检查下载地址是否可用 ${Font}"
		exit 4
	fi
}
...
#设置定时升级任务
modify_crontab(){
	echo -e "${OK} ${Blue} 配置每天凌晨自动升级V2ray内核任务 ${Font}"
	sleep 2
...
nginx_install(){
	${INS} install nginx -y
	if [[ -d /etc/nginx ]];then
		echo -e "${OK} ${Blue} nginx 安装完成 ${Font}"
		sleep 2
	else
...
#安装web伪装站点
web_install(){
	echo -e "${OK} ${Blue} 安装Website伪装站点 ${Font}"
	sleep 2
	mkdir /www
	wget https://www.moerats.com/usr/shell/V2_NG/V2_Website.tar.gz
	tar -zxvf V2_Website.tar.gz -C /www
	rm -f V2_Website.tar.gz
}
...
if [[ -e /etc/v2ray/v2ray.key ]]; then
	echo -e "${Info} ${Blue} 提示：检测到你的服务器已经存在ssl证书 为避免重复申请 脚本将自动跳过该步骤 ${Font}"
	echo -e "${Info} ${Blue} 如果你已更换新的域名 请按 ctrl+c 退出 然后执行 bash v.sh -q 强制重装 ${Font}"
	read -p "按 回车键 继续 …… "
	main_ssloff
else
	main_sslon
...
rm_userjson(){
	rm -rf /www/s
	echo -e "${OK} ${Blue} 客户端配置文件 config.json 已从 Website 中删除 ${Font} "
	echo -e "${OK} ${Blue} 提示：如果忘记配置信息 可执行 bash v.sh -n 重新生成 ${Font} "
}
...
new_uuid(){
    ...
    judge "V2ray 重启"
    echo -e "${Info} ${Blue} 新的 用户id（UUID）: ${NEWUUID} ${Font} "
    echo -e "${Info} ${Blue} 新的 客户端配置文件下载地址（URL）：https://你的域名:端口/s/${newcamouflage}/config.json ${Font} "
}

#启动nginx和v2ray程序 加载配置
start_NV(){
